...
while its local copy is still uploading). They coordinate through a
`metadata.json` state file per ingest dir.
...
| **Uploader** | `host/uploader.py` | built | Separate process (`nix run .#uploader`): pushes verified ingest dirs to the cloud with rclone, then verifies against the remote's own metadata hashes (no download) and flips `metadata.json` to `uploaded`. Decoupled from the daemon. |
...
uploader later flips `metadata.json` `state` to `uploaded`.
...
`metadata.json`, so it fills for a card still present when its upload completes.
